...
For coding work in a git repository, start each new agent thread on its own branch
...
name. Use the configured agent branch prefix when there is one, for example
`agent/ABC-123-short-task`.
...
workspace, for example `.worktrees/agent-short-task`, instead of `/tmp` or
`/private/tmp`. This keeps file edits inside the agent workspace permissions and
...
When the host app already provides an isolated worktree for the thread, use that
